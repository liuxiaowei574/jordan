...
public class WarehouseEsealService  extends LSBaseService{
	 ...
	 * @return
	 */
	/*public PageList findWarehouseEseal(PageQuery<Map> pageQuery) {
		String queryString = "select warehouseEsealBO from LsWarehouseEsealBO warehouseEsealBO where 1=1 "
	...
	public JSONObject fromObjectList(PageQuery<Map> pageQuery,JsonConfig jsonConfig,boolean ignoreDefaultExcludes) {
		PageList<LsWarehouseEsealBO> pageList = findWarehouseEseal(pageQuery);
	...
	public JSONObject fromObjectList(PageQuery<Map> pageQuery,JsonConfig jsonConfig,boolean ignoreDefaultExcludes){
		String queryString = 
				"select e,d from LsWarehouseEsealBO e,LsSystemDepartmentBO d where 1=1"
								+ " and e.belongTo=d.organizationId "
								+"/~ and e.esealNumber like '%[esealNumber]%' ~/"
								+"/~ and e.esealStatus like '%[esealStatus]%' ~/"
	...
	//调度的随机查询
		...
		public List getEseal(int m){
			return warehouseEsealDao.getEsealList(m);
		}
