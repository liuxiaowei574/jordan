 ...
 * 作者： 赵苏阳
 *
 * 描述：<p>设备调度的记录即“保存”相关信息 Service</p>
 * 创建时间：2016年6月12日
 */
...
public class DispatchRecordService extends LSBaseService {
	...
	public JSONObject fromObjectList(PageQuery<Map> pageQuery,JsonConfig jsonConfig,boolean ignoreDefaultExcludes,String organizationId) {
		
		String queryString = 
				"select e,d,f from LsWarehouseDeviceDispatchBO e,LsSystemDepartmentBO d,LsSystemDepartmentBO f where 1=1"
								+ " and e.fromPort= '" + organizationId + "'"
								+ " and e.fromPort=d.organizationId "
								+ " and e.toPort=f.organizationId "
								+"/~ and e.toPort like '%[toPort]%' ~/"
								+"/~ and e.fromPort like '%[fromPort]%' ~/"
		...
		if (queryList != null && queryList.size() > 0){
			for (Object obj : queryList) {
				...
				/*BeanUtils.copyProperties((LsWarehouseDeviceDispatchBO) objs[0], dispatchRecordDepartmentVO);
				BeanUtils.copyProperties((LsSystemDepartmentBO) objs[1], dispatchRecordDepartmentVO);*/
				if(objs[0]!=null){
