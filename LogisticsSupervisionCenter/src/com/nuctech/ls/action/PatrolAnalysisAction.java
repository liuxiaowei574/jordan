 ...
 */
@Namespace("/analysis")
public class PatrolAnalysisAction extends LSBaseAction {

    /**
     * 
     */
    private static final long serialVersionUID = 1785639145635362459L;
    protected static final String DEFAULT_SORT_COLUMNS = "user.userId ASC";
    ...
    @Action(value = "patrolAnalysis",
            results = { @Result(name = "success", location = "/analysis/patrolAnalysis.jsp") })
    public String performanceAnalysis() {
        pageQuery = this.newPageQuery(DEFAULT_SORT_COLUMNS);
        pageQuery = dealDateParam(pageQuery);
        userList = userService.findPatrolUsers(pageQuery);
        userArr.clear();
        if (userList != null) {
            userArr.addAll(userList);
        }

        List<PerformanceAnalysisVo> pageList = performanceAnalysisService.findPatorlAnalysis(pageQuery);
    ...
    @Action(value = "queryPatrolEcharts")
