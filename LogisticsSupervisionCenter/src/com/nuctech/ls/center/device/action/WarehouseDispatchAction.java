...
public class WarehouseDispatchAction extends LSBaseAction {
	 ...
	 * @return
	 */
	@Action(value = "toList", results = { @Result(name = "success", location = "/device/dispatchhandle/DeviceDispatchIndex.jsp") })
	...
	public void list() throws IOException {
		...
		JSONObject retJson = dispatchRecordService.fromObjectList(pageQuery, null, false,sessionUser.getOrganizationId());
	...
	public String deviceDispatchIndex() throws Exception {
		...
		public void RandomElockChoose() throws IOException {
			...
			public void RandomEsealChoose() throws IOException {
				...
				int m = Integer.parseInt(n);
				randomEsealList = warehouseEsealService.getEseal(m);
			...
			public void RandomSensorChoose() throws IOException {
				...
				randomSensorList = warehouseSensorService.getSensor(m);
	 ...
	 * @throws Exception
	 */
	@Action(value = "deviceDispatch", results = { @Result(name = "success", location = "/device/dispatchhandle/DeviceDispatchIndex.jsp"),
	...
	public String deviceDispatch() throws Exception {
		try {
			...
			//修改调度状态从0改为1；
			dispatchRecordBo.setDispatchStatus(dispatchStatus);
			...
			if (numbers != null &&!"".equals(numbers)) {
				...
				for (int i = 0; i < numbersList.length; i++) {
					...
					dispatchDetailBo.setRecviceStatus(recviceStatus);// 接收状态(调度时，状态为未接收)
			...
			if (esealNum != null && !"".equals(esealNum) ) {
				...
				for(int i = 0;i<esealNumList.length;i++){
					System.out.println(esealNumList.length);
					...
					dispatchDetailBo.setRecviceStatus(recviceStatus);
			...
			if(sensorNum != null && !"".equals(sensorNum)){
				...
				for(int i = 0;i<sensorNumList.length;i++){
					System.out.println(sensorNumList.length);
					dispatchDetailBo.setDetailId(generatePrimaryKey());
					dispatchDetailBo.setDispatchId(dispacthId);
